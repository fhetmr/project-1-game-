...
void update_building_logic(bool clicked) {
    ...
    // Б) З гравцем
    Box playerBox = {
        cam.x - 0.5f, cam.y - 1.5f, cam.z - 0.5f,
        cam.x + 0.5f, cam.y + 0.5f, cam.z + 0.5f
    };
    if (check_aabb_collision(previewBox, playerBox)) isColliding = true;

    // 4. Малювання привида
    glEnable(GL_BLEND);
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    
    if (isColliding) glColor4f(1.0f, 0.0f, 0.0f, 0.5f); // Червоний
    else             glColor4f(0.0f, 1.0f, 0.5f, 0.4f); // Зелений

    glPushMatrix();
        glTranslatef(gx, gy, gz);
        glRotatef(buildRotation, 0.0f, 1.0f, 0.0f);
        glScalef(1.5f, 1.5f, 1.5f); 
        
        // Pivot Fix
        float ox = (buildSelection->bounds.minX + buildSelection->bounds.maxX) / 2.0f;
        float oz = (buildSelection->bounds.minZ + buildSelection->bounds.maxZ) / 2.0f;
        glTranslatef(-ox, 0, -oz);

        glBindVertexArray(buildSelection->vao);
        glDrawArrays(GL_TRIANGLES, 0, buildSelection->vertexCount);
    glPopMatrix();
    ...
    if (clicked && !isColliding && buildingCount < 500) {
        ...
        buildings[buildingCount].worldBounds = previewBox; // Зберігаємо повернуту колізію
        buildingCount++;
    }
}
