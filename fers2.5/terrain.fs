...
//
// ---------- NOISE (великі плавні плями як на фото)
//
float hash(vec2 p){
...
{
    // =========================================================
    // 1. ТЕКСТУРА + ЗЕЛЕНИЙ ТОН (як на фото)
    // =========================================================
    vec3 tex = texture(grassTex, TexCoord).rgb;

    tex.r *= 0.8;     // прибираємо жовтизну
    tex.g *= 1.15;    // підсилюємо зелений
    tex.b *= 0.95;

    vec3 deepGreen = vec3(0.03, 0.28, 0.07);
    tex = mix(tex, deepGreen, 0.35);

    // великі природні переходи кольору
